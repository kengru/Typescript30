...
const reduced = inventors.reduce(
  (acc, current) => (acc += current.passed - current.year),
  0
);
console.log(reduced);

// 5. Sort the inventors by years lived
const sortedYears = inventors.sort(
...
// const links = Array.from(category.querySelectorAll<HTMLAnchorElement>("a"));
// const de = links.map((link) => link.textContent);
// const final = de.filter((data) => data.includes("de"));

// 7. sort Exercise
// Sort the people alphabetically by last name
const sortedPeople = people.sort((prev, next) => {
  const [pLast, pFirst] = prev.split(", ");
  const [nLast, nFirst] = next.split(", ");
